...
//The code that does cross over mutation
public class GeneticAlgorithm {
    ...
    Population crossoverPopulation(Population population){
        ...
        IntStream.range(0, TimetableScheduler.NUMB_OF_ELITE_SCHEDULES).forEach(x -> crossoverPopulation.getSchedules().set(x, population.getSchedules().get(x)));
        IntStream.range(TimetableScheduler.NUMB_OF_ELITE_SCHEDULES, population.getSchedules().size()).forEach(x -> {
            if(TimetableScheduler.CROSSOVER_RATE > Math.random()){
    ...
    Population mutatePopulation(Population population){
        ...
        IntStream.range(0, TimetableScheduler.NUMB_OF_ELITE_SCHEDULES).forEach(x -> schedules.set(x, population.getSchedules().get(x)));
    ...
    Schedule mutateSchedule(Schedule mutateSchedule){
        ...
        IntStream.range(0, mutateSchedule.getClasses().size()).forEach(x -> {
            if(TimetableScheduler.MUTATION_RATE > Math.random())mutateSchedule.getClasses().set(x, schedule.getClasses().get(x));
        });
        return mutateSchedule;
    }
    
    Population selectTournamentPopulation(Population population){
        Population tournamentPopulation = new Population(TimetableScheduler.TOURNAMENT_SELECTION_SIZE, data);
        IntStream.range(0, TimetableScheduler.TOURNAMENT_SELECTION_SIZE).forEach(x -> {
